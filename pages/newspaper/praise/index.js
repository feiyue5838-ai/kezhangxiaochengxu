...
// fallback：praise.js 本地 categories + generateContent
let _categoriesFromApi = null;

Page({
  data: {
    selectedCategory: '',
    categories: praiseConfig.categories.map(cat => ({
      id: cat.id,
      name: cat.name,
      desc: cat.desc,
      color: cat.color,
  ...
  async _loadFromApi() {
    ...
    try {
      const res = await api.getPraiseTemplates();
      ...
      console.warn('[praise] API 调用失败，使用前端硬编码兜底', _e);
    }
  ...
  selectItem(e) {
    ...
    const content = (item.content && item.content.trim())
      ? item.content
      : (praiseConfig.generateContent(name) || '');
