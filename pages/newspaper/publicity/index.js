// pages/newspaper/publicity/index.js
const common = require('../../../utils/common.js');
const publicityConfig = require('../../../utils/publicity.js');
const api = require('../../../utils/api.js');

// API 返回分组：{ id, name, color, hot, total, docs: [{ name, content }] }
// fallback：publicity.js 本地 categories + generateContent
let _categoriesFromApi = null;

Page({
  data: {
    selectedCategory: '',
    categories: publicityConfig.categories.map(cat => ({
      id: cat.id,
      name: cat.name,
      desc: cat.desc,
      color: cat.color,
  ...
  async _loadFromApi() {
    ...
    try {
      const res = await api.getPublicityTemplates();
      ...
      console.warn('[publicity] API 调用失败，使用前端硬编码兜底', _e);
    }
  ...
  selectItem(e) {
    ...
    const content = (item.content && item.content.trim())
      ? item.content
      : (publicityConfig.generateContent(name) || '');
