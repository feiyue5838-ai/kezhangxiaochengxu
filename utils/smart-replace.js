...
function doSmartReplace(content, categoryName) {
  ...
  // ══════════════════════════════════════════════════════
  // 【个人证件专项】证号/证书编号/联系电话错误修复
  // 必须在通用 XXX/XXXX 兜底规则之前执行，防止被"张三"截断
  // ══════════════════════════════════════════════════════
  if (categoryName === '个人证件') {
    // 1. 证号类字段 → 证号格式示例（先于通用XXX兜底，保护证号不被替换为人名）
    var certFields = ['证号', '证书编号', '证件编号', '权证编号', '编号', '票据编号', '备案编号'];
    certFields.forEach(function(f) {
      r = r.replace(new RegExp(f + '：XXX', 'g'), f + '：（示例：12345678）');
    });
    // 2. 联系电话：XXXX → 座机格式（8位以上数字占位，防止被"XX"截断）
    r = r.replace(/联系电话：XXXXXXXX(?![年\d一-龥])/g, '联系电话：（示例：0755-12345678）');
    r = r.replace(/联系电话：XXXX(?![年\d一-龥])/g, '联系电话：（示例：0755-12345678）');
  }
